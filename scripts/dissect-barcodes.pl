...
# Read tag sequences:
my $pepA   = &ReadBarcodesWithPosFromFasta("$fastaDir/epiA.fa",    0);  # do not rev comp
my $pepB   = &ReadBarcodesWithPosFromFasta("$fastaDir/epiB.fa",    1);  # rev comp each sequence
my $anneal = &ReadBarcodesWithPosFromFasta("$fastaDir/anneal.fa",    0);  
my $primA  = &ReadBarcodesWithPosFromFasta("$fastaDir/primA.fa", 0);  
# my $primB  = &ReadBarcodesWithPosFromFasta("$fastaDir/primB.fa",     0); 
my $primB  = &ReadBarcodesWithPosFromFasta("$fastaDir/primB.fa",     1); 
...
my $samTags = &ReadBarcodesFromFasta2("$fastaDir/sample.fa", 0);  
...
# Header
print join("\t", ("read_id", "read_quality", "sample_id", "nw_score", "pepA", "pepB", "sample_seq", "primA_seq", "N6A", "pepA_seq", "anneal_seq", "pepB_seq", "N6B", "primB_seq")), "\n";

while(defined(my $line = <IN>)) {
	chomp $line;
	# print STDERR "Read line $.\n" if $. =~ m/000$/;
	my ($read_id, $a, $b, $read, $qual) = split("\t", $line);
	my ($qual_score) = MeanQuality($qual);

  my %Pos = ();
 
  # # constant sequences:
	$Pos{primA}  = SWalignTarget($read, $primA,  $buffer);
  $Pos{anneal} = SWalignTarget($read, $anneal, $buffer);
  $Pos{primB}  = SWalignTarget($read, $primB,  $buffer);

  # variables sequences:
	$Pos{pepA} = SWalignTarget($read, ${$pepA}{$a}, $buffer);
	$Pos{pepB} = SWalignTarget($read, ${$pepB}{$b}, $buffer);
  ...
  print join("\t", ($read_id, $qual_score, $sample_id, $nw_score, $a, $b, $sample, $fwdpA, $N1, $codA, $ann, $codB, $N2, $pB)), "\n";
