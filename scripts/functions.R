...
read.annotation <- function(f) {
  ...
  if(isText) {
    ...
    if(any.duplicated.barcodes(anno_tmp[,1])) stop()
  ...
  if(isExcel) {
    ...
    anno <- lapply(sheets, function(i) {
      ...
      if(any.duplicated.barcodes(anno_tmp[,1], filename = i)) stop()
...
plotCounts <- function(x, cols) {
  ...
  p <- p + geom_point(aes(shape = Enriched))
  # p <- p + theme_bw()
...
plotReadsPerSample <- function(tab, sid) {
  ...
  g <- ggplot(data = df,
    ...
    coord_flip() +
    geom_text(vjust = 0.5, hjust = 1, size = 2.5, nudge_y = -nudge) +
    geom_text(aes(x = Key, label = sample, y = 0),
              vjust = 0.5, hjust = 0, size = 2.5, nudge_y = nudge) +
...
calcBarplotHeight <- function(n, base = 2, div = 6) {
  if(length(n) > 1) {
    n <- length(n)
  }
  floor(n/div) + base
...
printGgplot <- function(p, file, dir, height = 7, width = 7) {
  # avoid too large pngs, which will crash (arbitrary cutoff)
  if(height/width < 100) {
    png(file.path(dir, paste0(file, ".png")),
      height = round(600*height/width), width = 600)
      print(p)
      dev.off()
  }
  
  pdf(file.path(dir, paste0(file, ".pdf")),
